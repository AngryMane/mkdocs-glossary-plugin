...
def check_pandoc():
    ...
    if not which_pandoc_result:
        print(
            "\033[31m"
            + "[WARNING]This plugin needs pandoc>=2.11. Please install pandoc."
            + "\033[0m"
        )
        return
    completed_process: CompletedProcess = run(
    ...
    if major_version < 2 or major_version == 2 and minor_version < 11:
        print(
            "\033[31m"
            + f"[WARNING]The version of your pandoc is {pandoc_version_text}, but this plugin needs 2.11 or later."
            + "\033[0m"
        )
        return
