...
class GlossaryPlugin(BasePlugin):
    ...
    def on_config(self: "GlossaryPlugin", config: Config) -> Config:
        ...
        self.__glossary_dirs = [
            self.__docs_dir + "/" + x for x in self.config["glossary_dirs"]
        ]
        ...
        # validation
        for invalid_glossary_dir in [
            x for x in self.__glossary_dirs if not self.validate_glossary_path(x)
        ]:
    ...
    def on_files(self: "GlossaryPlugin", files: Files, config: Config) -> Files:
        ...
        md_files: List[File] = [
            x
            for x in files._files
            if x.abs_src_path.endswith(self.__suffix) and self.__is_in_glossary_dirs(x)
        ]
    ...
    def on_nav(
        self: "GlossaryPlugin", nav: Navigation, config: Config, files: Files
    ) -> Navigation:
        """
    ...
    def on_page_markdown(
        self: "GlossaryPlugin", markdown: str, page: Page, config: Config, files: Files
    ) -> str:
        """
    ...
    def validate_glossary_path(self: "GlossaryPlugin", glossary_path: str) -> bool:
        return (
            ...
            if not os.path.isdir(glossary_path)
            else True
        )

    def __convert_file_to_words(
        self: "GlossaryPlugin", config: Config, file: File
    ) -> List[Word]:
        result: List[Word] = []
